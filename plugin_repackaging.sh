...
while getopts "p:s:R" opt; do
	case "$opt" in
		p)
			RAW_PLATFORM="${OPTARG}"
			if [[ "${OPTARG}" == *"aarch64"* || "${OPTARG}" == *"arm64"* ]]; then
				PIP_PLATFORM="--platform manylinux_2_17_aarch64 --platform manylinux_2_28_aarch64 --only-binary=:all:"
			else
				PIP_PLATFORM="--platform manylinux_2_17_x86_64 --platform manylinux_2_28_x86_64 --only-binary=:all:"
			fi
			;;
